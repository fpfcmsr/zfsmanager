...
#options and defaults when creating new pool or filesystem
sub create_opts
{
...
sub list_zpools
{
	my ($pool) = @_;
	my %hash=();
	$list=&backquote_command("zpool list -H -o name,$config{'list_zpool'} $pool");
...
{
	my ($snap) = @_;
	$list=&backquote_command("zfs list -t snapshot -H -o name,$config{'list_snap'} -s creation $snap");
	$idx = 0;
	open my $fh, "<", \$list;
	while (my $line =<$fh>) {
		chomp ($line);
...
sub get_alerts
{
	my $alerts = &backquote_command("zpool status -x");
	my %status = ();
	my $pool = ();
...
sub zpool_status
{
	...
	my $parent = "pool";
	my %status = ();
	my $cmd=&backquote_command("zpool status $pool");
	...
	if (index($cmd, "status: ") != -1) {
		...
		if (index($cmd, "  see: ") != -1) {
			...
			($status{0}{action}, $cmdout) = split("  scan: ", $cmdout);
		}
	} else {
...
sub zfs_get
{
	...
	my %hash=();
	my $get=&backquote_command("zfs get -H $property $zfs");
	open my $fh, "<", \$get;
	while (my $line =<$fh>) {
		chomp ($line);
...
sub zpool_get
{
	...
	if (~$property) {my $property="all";}
	my %hash=();
	my $get=&backquote_command("zpool get -H $property $pool");
...
sub zpool_imports
{
	...
	if ($dir) { $dir = '-d '.$dir; }
	my %status = ();
	my $cmd = &backquote_command("zpool import $dir $destoryed");
	$count = 0;
...
{
	my ($snap, $parent) = @_;
	my @array = split("\n", &backquote_command("zfs diff -FH $snap"));
	return @array;
}
...
sub ui_zfs_list
{
	...
	my %zfs = list_zfs($zfs);
	if ($action eq undef) {
		$action = "status.cgi?zfs=";
	}
	@props = split(/,/, $config{list_zfs});
	print &ui_columns_start([ "file system", @props ]);
	foreach $key (sort(keys %zfs)) {
		@vals = ();
		if ($zfs{$key}{'mountpoint'}) {
			$zfs{$key}{'mountpoint'} = "<a href='../filemin/index.cgi?path=".urlize($zfs{$key}{mountpoint})."'>$zfs{$key}{mountpoint}</a>";
		}
		foreach $prop (@props) {
			push (@vals, $zfs{$key}{$prop});
		}
		print &ui_columns_row(["<a href='$action$key'>$key</a>", @vals ]);
	}
	print &ui_columns_end();
